...
** Last update Sun Sep 11 21:11:18 2016 jeremy thiriez
*/
...
char		**my_switch_plit(char **dest, char *str, int *i, int *j)
{
  if ((dest = realloc_tab(dest)) == NULL)
    return (NULL);
  if (def_in_ope(str, *i) == 0)
    {
      if ((dest[*j] = cpy_str(str, &i)) == NULL)
	return (NULL);
    }
  else
    if ((dest[*j] = cpy_ope(str, &i)) == NULL)
      return (NULL);
  *j = *j + 1;
  dest[*j] = 0;
  return (dest);
  ...
  while (str[i])
    {
      if (str[i] != ' ' && str[i] != '\t')
	{
	  if ((dest = my_switch_plit(dest, str, &i, &j)) == NULL)
	    return (NULL);
	}
      else
	i += 1;
